...
String sql ="SELECT * FROM iplog WHERE  NOT out_country='India(IN)'";
...
%>
  ...
  int japnum=0;
   ...
   try{
        ...
        ResultSet rs3 = st.executeQuery("SELECT COUNT(out_country) AS count FROM iplog WHERE out_country='Japan(JP)'");

        while(rs3.next()){
            japnum = rs3.getInt("count");
        }
 ...
 int ausnum=0;
       try{
        Statement st1 = connection.createStatement();
            ...
            ResultSet rs4 = st1.executeQuery("SELECT COUNT(out_country) AS count2 FROM iplog WHERE NOT out_country='Japan(JP)' ");

        while(rs4.next()){
           ausnum = rs4.getInt("count2");
        }
     ...
     int usnum=0;
       ...
       try{
        Statement st1 = connection.createStatement();
            ...
            ResultSet rs4 = st1.executeQuery("SELECT COUNT(out_country) AS count2 FROM iplog WHERE out_country='United States(US)' ");

        while(rs4.next()){
           usnum = rs4.getInt("count2");
        }
...
function drawChart1() {
        ...
        ['COUNTRY', 'RECORDS',{ role: 'style' }],
         ['USA', d1,' color:red'],
        ['JAPAN',b1,'color: lime'],
        ['AUSTRALIA',a1,'color:pink' ],
        ['ASIA/PACIFIC',c1,'color:green']
      ]);
